...
public class UsuarioBD
{
    ...
    public DataSet UsuarioAdm()
    {
        ...
        objComando = Mapped.Command("select u.usu_id as ID, CONCAT(pf.perf_nome, ' ', pf.perf_sobrenome) as Nome, u.usu_rg as RG, u.usu_registro as Registro, p.perm_colunista as Colunista, p.perm_moderador as Moderador, p.perm_forumPost as 'Postagem Fórum', p.perm_forumResp as 'Responder Fórum', p.perm_colunaResp as 'Responder Coluna', p.perm_administrador as Administrador from tbl_usuario u inner join tbl_permissoes p on u.usu_id = p.usu_id inner join tbl_perfil pf on u.usu_id = pf.usu_id order by pf.perf_nome", objConexao);
    ...
    public void Insert(Usuario usu)
    {
        ...
        PermissoesBD permissaobd = new PermissoesBD();
        ...
        perfilbd.Insert(usu);
        usu.PerfilC.IdPerfil = perfilbd.RetornaID(usu.IdUsuario);

        usu.Permissoes = new Permissoes();
        permissaobd.Insert(usu);

        List<Interesses>  lstInteresses = new List<Interesses>();
        usu.PerfilC.Interesses = new Interesses();
        for (int i = 1; i <= 5; i++)
        {
            Interesses interesse = new Interesses();
            interesse.InteressesTipo = "Lazer";
            interesse.IndexInteresse = i;
            lstInteresses.Add(interesse);
        }
        for (int i = 1; i <= 5; i++)
        {
            Interesses interesse = new Interesses();
            interesse.InteressesTipo = "Livros";
            interesse.IndexInteresse = i;
            lstInteresses.Add(interesse);
        }
        for (int i = 1; i <= 5; i++)
        {
            Interesses interesse = new Interesses();
            interesse.InteressesTipo = "Musica";
            interesse.IndexInteresse = i;
            lstInteresses.Add(interesse);
        }
        for (int i = 1; i <= 5; i++)
        {
            Interesses interesse = new Interesses();
            interesse.InteressesTipo = "Esporte";
            interesse.IndexInteresse = i;
            lstInteresses.Add(interesse);
        }
        for (int i = 1; i <= 5; i++)
        {
            Interesses interesse = new Interesses();
            interesse.InteressesTipo = "Filme";
            interesse.IndexInteresse = i;
            lstInteresses.Add(interesse);
        }

        interessesbd.InsertNovoUsr(lstInteresses, usu.PerfilC.IdPerfil);



        List<MiniCv> lstMiniCv = new List<MiniCv>();
        usu.PerfilC.Minicv = new MiniCv();
        for (int i = 1; i <= 5; i++)
        {
            MiniCv minicv = new MiniCv();
            minicv.MiniCvTipo = "curso";
            minicv.IndexMiniCv = i;
            lstMiniCv.Add(minicv);
        }
        for (int i = 1; i <= 5; i++)
        {
            MiniCv minicv = new MiniCv();
            minicv.MiniCvTipo = "expprofissional";
            minicv.IndexMiniCv = i;
            lstMiniCv.Add(minicv);
        }
        for (int i = 1; i <= 1; i++)
        ...
        {
            MiniCv minicv = new MiniCv();
            minicv.MiniCvTipo = "cursando";
            minicv.IndexMiniCv = i;
            lstMiniCv.Add(minicv);
        }

        minicvbd.InsertNovoUsr(lstMiniCv, usu.PerfilC.IdPerfil);
